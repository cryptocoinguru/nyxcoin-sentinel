...
def is_valid_nyx_address(address, network='mainnet'):
    ...
    nyx_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Nyx address
        return False

    if (address_version != nyx_version):
        return False
...
# shims 'til we can fix the nyxd side
def SHIM_serialise_for_nyxd(sentinel_hex):
    from models import NYXD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = NYXD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in nyxd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # nyxd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    nyxd_hex = serialise(obj)
    return nyxd_hex


# shims 'til we can fix the nyxd side
def SHIM_deserialise_from_nyxd(nyxd_hex):
    from models import NYXD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(nyxd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('nyx.conf')
