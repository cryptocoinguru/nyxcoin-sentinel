...
def has_nyx_conf():
    ...
    valid_nyx_conf = False

    # ensure nyx_conf exists & readable
    #
    # if not, print a message stating that Nyx Core must be installed and
    # configured, including JSONRPC access in nyx.conf
    try:
        f = io.open(config.nyx_conf)
        valid_nyx_conf = True
    except IOError as e:
        print(e)

    return valid_nyx_conf
...
def main():
    ...
    if not has_nyx_conf():
        print("Nyx must be installed and configured, including JSONRPC access in nyx.conf")
        sys.exit(1)
