...
class GovernanceObject(BaseModel):
    ...
    # sync nyxd gobject list with our local relational DB backend
    @classmethod
    def sync(self, nyxd):
        golist = nyxd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_nyxd(nyxd, item)
    ...
    @classmethod
    def import_gobject_from_nyxd(self, nyxd, rec):
        import decimal
        import nyxlib
        import inflection
        ...
        # shim/nyxd conversion
        object_hex = nyxlib.SHIM_deserialise_from_nyxd(object_hex)
        objects = nyxlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from nyxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from nyxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from nyxd - Nyxd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(nyxd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from nyxd! %s" % e)
            govobj.vote_delete(nyxd)
            return (govobj, None)
    ...
    def vote_delete(self, nyxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(nyxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, nyxd, signal, outcome):
        import nyxlib
        ...
        output = nyxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = nyxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(nyxd, signal)

    def sync_network_vote(self, nyxd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = nyxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = NYXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 nyx addr, non-multisig
            if not nyxlib.is_valid_nyx_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Nyx address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (nyxlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/NyxDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import nyxlib
        obj_data = nyxlib.SHIM_serialise_for_nyxd(self.serialise())
    ...
    def prepare(self, nyxd):
        try:
            object_hash = nyxd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = NYXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not nyxlib.is_valid_nyx_address(addr, config.network):
    ...
    def hash(self):
        import nyxlib
        return nyxlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = NYXD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, nyxd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - nyxd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, nyxd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - nyxd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, nyxd):
        now = int(time.time())
        return (self.created_at < (now - nyxd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, nyxd):
        if self.is_expired(nyxd):
            return False

        return True

    def is_deletable(self, nyxd):
        if self.is_expired(nyxd):
            return True
