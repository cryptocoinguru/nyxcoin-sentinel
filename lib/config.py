...
sentinel_cfg = NyxConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_nyxd_proto_version_with_sentinel_ping = 70207


def get_nyx_conf():
    home = os.environ.get('HOME')

    nyx_conf = os.path.join(home, ".nyx/nyx.conf")
    if sys.platform == 'darwin':
        nyx_conf = os.path.join(home, "Library/Application Support/Nyx/nyx.conf")

    nyx_conf = sentinel_cfg.get('nyx_conf', nyx_conf)
