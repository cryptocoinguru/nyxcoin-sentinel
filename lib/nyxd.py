...
class NyxDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some nyxd methods
    ...
    @classmethod
    def from_nyx_conf(self, nyx_dot_conf):
        from nyx_config import NyxConfig
        config_text = NyxConfig.slurp_config_file(nyx_dot_conf)
        creds = NyxConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from nyxlib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = nyxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = nyxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = nyxlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_nyxd_proto_version_with_sentinel_ping)
